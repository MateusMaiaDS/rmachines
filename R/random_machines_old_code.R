...
random_machines<-function(formula,#Formula that will be used
  ...
  predict<-purrr::map(early_model,~predict(.x,newdata=test))
  ...
  predict<-purrr::map(models,~predict(.x,newdata=test))
  
  #Prediction of OOB samples
  predict_oobg<-purrr::map2(models,out_of_bag,~predict(.x,newdata=.y))
  ...
  predict_new<-purrr::map(models,~predict(.x,newdata=test_new))
